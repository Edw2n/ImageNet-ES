...
def auc_and_fpr_recall(conf, label, tpr_th):
    ...
    fpr_list, tpr_list, thresholds = metrics.roc_curve(ood_indicator, -conf) # positive 면 ood여서 사실 conf가 더 큰값이 들어가야 하는게 맞는데 roc_curve 개념에서는... 근데 ood conf가 보통 더 작으니까 그 크기관계를 반전시켜주기 위해서 다르게 씀
